...
func TestCreateMembership(t *testing.T) {
	t.Run("멤버십을 생성한다.", func(t *testing.T) {
		app := NewApplication(Service{repository: *NewRepository(map[string]Membership{})})
...
func TestUpdate(t *testing.T) {
	...
	t.Run("수정하려는 사용자의 이름이 이미 존재하는 사용자 이름이라면 예외 처리한다.", func(t *testing.T) {
		id := uuid.New().String()
		app := NewApplication(Service{repository: *NewRepository(map[string]Membership{id: {id, "jenny", "naver"}, "33":{"33", "genie", "naver"}})})
		req := UpdateRequest{id, "genie", "naver"}
		res, err := app.Update(req)
		assert.Empty(t, res)
		assert.EqualError(t, err, "이미 등록된 사용자 이름입니다")
	})
	...
	t.Run("주어진 멤버쉽 타입이 아닌 경우, 예외 처리한다.", func(t *testing.T) {
		app := NewApplication(Service{repository: *NewRepository(map[string]Membership{})})
		req := UpdateRequest{"1", "jenny", "kakao"}
		res, err := app.Update(req)
		assert.Empty(t, res)
		assert.EqualError(t, err, "해당 멤버십 타입은 유효하지 않습니다")
	})
